...
let updateSummary=[
	{
		...
		<p class='warning'>FYI - This update to the  extension makes some internal changes to its internal data structures. As a precaution, a '<i>just-in-case</i>' back-up of your current class names and student lists was saved to your downloads directory.</p>
...
let meetStart=[
	{
		version:'', 
		title:'Le Cours vient de commencer', 
		intro:'Veuillez confirmer que les paramètres suivants sont corrects avant de commencer:', 
		body:``, 
		footer:"Cliquez sur la coche ✔ à côté de l'icône vidéo ci-dessous pour masquer / afficher la boîte de dialogue Présence."
	},
]
...
function showMeetingStarted(){
	...
	let hpb=document.getElementById( 'help-page-body' )
	addElement(hpb,'li','','','',"Le cours a commencé à <span id='current-start-time'></span> et il est maintenant <span id='current-time'></span>.<br/>Si l'heure de déébut n'est pas correct, cliquez ici <span id='resetStartTime'></span>.")

	addElement(hpb,'li','','','',"<b>Rappel</b>: pour prendre et surveiller correctement les présences, tous les élèves<b><u>doivent</u></b>être visible dans votre fenêtre Meet pendant toute la durée de votre cours")			
	if(nstu<49){
		write2log( 'Class smaller than 49... use Tiled layout ' + nstu )
		addElement(hpb,'p','','','',"Votre classe est suffisamment petite pour utiliser la mise en page en mosaïque intégrée. <br/> Veuillez confirmer que vous utilisez cette mise en page en cliquant sur les points de suspension verticaux \ '& vellip; \' dans le coin inférieur droit de l'écran et en choisissant `Modifier la mise en page `(ou assurez-vous que l'extension Grid View (fix) est installée et activée).")
	}
	else{
		addElement(hpb,'p','','','',"Votre classe compte plus de 16 élèves, vous devez donc <b><u>doit</u> </b> utiliser l’extension Grid View (correctif).")
		if(gmgv.length===0){
			write2log( 'Class larger than 49... Grid View is not installed' )
			addElement(hpb,'p','','','warning', "Il semble que vous n'ayez pas installé (ou que vous ayez peut-être désactivé) l'extension Grid View.")		
			...
			if(gmgv[0].innerHTML.indexOf('M10,4V8H14V4H10M16')==-1){
				...
				addElement(hpb,'li','','','warning',"Il semble que l'extension Grid View est installée mais peut ne pas être activée. Veuillez vérifier à nouveau pour vous assurer que Grid View est activé.")		
			}
			if(document.querySelector('[data-gmgv-setting="show-only-video"]').checked){
				write2log( 'Class larger than 16... Grid View is not enabled' )
				addElement(hpb,'li','','','warning',"L'option Afficher uniquement les participants avec l'option vidéo <b><u> ne peut pas</u> </b> être cochée car seuls ces élèves seront marqués comme présents!")		
			}
		}
	...
	addElement(hpb,'li','','','',"Si vous devez présenter du contenu depuis votre ordinateur, je vous suggère de partager une fenêtre distincte ou un deuxième écran.")		
